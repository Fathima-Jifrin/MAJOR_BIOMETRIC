...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($response['success'] && isset($response['data']['BitmapData'])) {
       $fingerprint_data = $response['data']['BitmapData'];

        // Prepare the database statement to save the fingerprint data
        $stmt = $conn->prepare("UPDATE aadhaar_data SET index_f = ? WHERE aadhaar_number = ?");
        $stmt->bind_param("ss", $fingerprint_data, $aadhaar_number);
            ...
            fetch("", { method: "POST", headers: { "Content-Type": "application/json" } })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById("result").innerHTML = `<span style="color:rgb(82, 236, 82);">Scan Successful!</span><br>`;
